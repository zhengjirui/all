...
public class OvernightBillActivity extends BaseActivity {
    ...
    //商品适配器
    private CommonAdapter<ProductListBean.ProductBean> productAdapter;
    ...
    private void kindData(HomeSearchResultBean result) {
        ...
        if (page == 1) {
            productAdapter = new CommonAdapter<ProductListBean.ProductBean>(mContext, productList, R.layout.item_program_product) {
                @Override
                public void setData(ViewHolder viewHolder, Object item) {
                    viewHolder.getView(R.id.iv_kaiqiang).setVisibility(View.VISIBLE);
                    ProductListBean.ProductBean bean = (ProductListBean.ProductBean) item;
